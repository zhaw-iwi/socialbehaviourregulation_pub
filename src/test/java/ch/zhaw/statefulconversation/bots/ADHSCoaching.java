...
import ch.zhaw.statefulconversation.socialbehaviourregulation.coaching.CoachingImpl;
...
class ADHSCoaching {
        ...
        @Test
        void setUp() {
                RegulationSystem regulationSystem = new CoachingImpl(true);

                Storage storage = new Storage();
                Decision trigger = new StaticDecision(
                                ADHSCoaching.PROMPT_TRIGGER);
                Decision guard = new StaticDecision(
                                ADHSCoaching.PROMPT_GUARD);
                Action action = new StaticExtractionAction(ADHSCoaching.PROMPT_ACTION,
                                storage,
                                "summary");
                ...
                State state = new RegulatedState(regulationSystem,
                                ADHSCoaching.PROMPT_STATE,
                                "Mental Health Support Interaction",
                                ADHSCoaching.PROMPT_STATE_STARTER,
                                List.of(transition));
                Agent agent = new Agent("Mental Health Support Companion (ADHS Coaching)",
