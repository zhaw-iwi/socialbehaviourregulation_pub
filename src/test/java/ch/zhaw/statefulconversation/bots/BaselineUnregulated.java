...
class BaselineUnregulated {
        ...
        private static final String PROMPT_TRIGGER = """
                        Review the user's last couple of messages in the following conversation.
        ...
        private static final String PROMPT_ACTION = """
                        Summarize the conversation, highlighting the user's attitude and well-being. Format the summary as a JSON object with meaningful attributes.
                        """;
        @Autowired
        ...
        void setUp() {
                ...
                Decision trigger = new StaticDecision(
                                BaselineUnregulated.PROMPT_TRIGGER);
                Decision guard = new StaticDecision(
                                BaselineUnregulated.PROMPT_GUARD);
                Action action = new StaticExtractionAction(BaselineUnregulated.PROMPT_ACTION, storage,
                                "summary");
                Transition transition = new Transition(List.of(trigger, guard), List.of(action), new Final());
                State state = new State(BaselineUnregulated.PROMPT_STATE,
                                "Mental Health Support Interaction",
                                BaselineUnregulated.PROMPT_STATE_STARTER,
                                List.of(transition));
                Agent agent = new Agent("Mental Health Support Companion (Baseline Unregulated)",
