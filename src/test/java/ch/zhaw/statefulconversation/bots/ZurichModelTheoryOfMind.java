...
import ch.zhaw.statefulconversation.socialbehaviourregulation.zurichmodel.PromptsProvider;
import ch.zhaw.statefulconversation.socialbehaviourregulation.zurichmodel.UserPromptsProvider;
import ch.zhaw.statefulconversation.socialbehaviourregulation.zurichmodel.ZurichModelImpl;

@SpringBootTest
class ZurichModelTheoryOfMind {
        ...
        @Test
        void setUp() {
                PromptsProvider.instanciate(UserPromptsProvider.class);
                RegulationSystem regulationSystem = new ZurichModelImpl(0, 0, 0);

                Storage storage = new Storage();
                Decision trigger = new StaticDecision(
                                ZurichModelTheoryOfMind.PROMPT_TRIGGER);
                Decision guard = new StaticDecision(
                                ZurichModelTheoryOfMind.PROMPT_GUARD);
                Action action = new StaticExtractionAction(ZurichModelTheoryOfMind.PROMPT_ACTION,
                                storage,
                                "summary");
                ...
                State state = new RegulatedState(regulationSystem,
                                ZurichModelTheoryOfMind.PROMPT_STATE,
                                "Mental Health Support Interaction",
                                ZurichModelTheoryOfMind.PROMPT_STATE_STARTER,
                                List.of(transition));
                Agent agent = new Agent("Mental Health Support Companion (Zurich Model Theory of Mind)",
